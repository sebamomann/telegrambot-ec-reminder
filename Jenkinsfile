            ...
            steps {
                script {
                    docker.withRegistry('http://localhost:34015') {
                        ...
                        sh "docker run --name ${name} -v /var/lib/docker/volumes/telegrambot-ec-reminder/_data/bot.env:/usr/src/app/.env -d ${image_name}"
                    }
                }
            }
        }
...
void updateStatus(String value) {
    sh 'curl -s "https://api.github.com/repos/sebamomann/telegrambot-ec-reminder/statuses/$GIT_COMMIT" \\\n' +
